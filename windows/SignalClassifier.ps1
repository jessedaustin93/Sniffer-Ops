...
function Get-WifiSignalExplanation {
    ...
    return New-SignalExplanation -Category "WiFi" -SpecificType "WiFi wireless device" -Confidence "Low" -Evidence $evidence -Meaning "Windows can see the network, but the SSID/security/channel do not identify a more specific device class." -NextStep "Use signal strength, location changes, BSSID/vendor lookup, or the phone app to narrow down the physical source."
}
